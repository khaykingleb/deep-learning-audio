...
class CosineAnnealingWarmupLRScheduler(LRScheduler):
    ...
    def __init__(
        ...
        """
        self.__check_args(first_cycle_steps, cycle_multiplier, min_lr, max_lr, warmup_steps, gamma)
    ...
    def __check_args(
        ...
        if max_lr <= min_lr:
            raise ValueError("Maximum learning rate should be greater than minimum learning rate.")
        if warmup_steps >= first_cycle_steps:
            raise ValueError("Warmup steps should be less than first cycle steps.")
        if gamma <= 0:
    ...
    def step(self, epoch=None):
        if epoch is None:
            ...
            if self.step_in_cycle >= self.cur_cycle_steps:
                ...
                self.cur_cycle_steps = int((self.cur_cycle_steps - self.warmup_steps) * self.cycle_multiplier) + self.warmup_steps
        else:
            if epoch >= self.first_cycle_steps:
                if self.cycle_multiplier == 1.:
                    ...
                    n = int(math.log((epoch / self.first_cycle_steps * (self.cycle_multiplier - 1) + 1), self.cycle_multiplier))
                    self.cycle = n
                    self.step_in_cycle = epoch - int(self.first_cycle_steps * (self.cycle_multiplier ** n - 1) / (self.cycle_multiplier - 1))
                    self.cur_cycle_steps = self.first_cycle_steps * self.cycle_multiplier ** n
            else:
